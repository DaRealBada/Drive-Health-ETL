...
function validateEnvelope(envelope) {
  ...
  if (isNaN(Date.parse(envelope.occurred_at))) throw new Error('occurred_at must be a valid ISO date string');
  return true;
}
...
function computeIdempotencyKey(envelope) {
  ...
  if (!key) throw new Error('No idempotency key found');
  return key;
}
...
// --- MAIN ENDPOINT ---
app.get('/', (req, res) => res.send('ETL Service is running!'));

app.post('/pubsub', async (req, res) => {
  let idempotencyKey, envelope;
  try {
    ...
    // UPDATED: Refined log with only IDs
    const logMetadata = {
        idempotencyKey,
    ...
    if (!shouldSample(idempotencyKey, AUDIT_RATE)) {
      logger.info('Event not sampled', { ...logMetadata, sampled: false });
    ...
    await bigquery.dataset(BQ_DATASET).table(BQ_TABLE).insert([rowToInsert]);

    // UPDATED: Refined success log
    logger.info('BigQuery insert success', { ...logMetadata, insert_status: 'SUCCESS' });
    res.status(204).send();

  } catch (error) {
    if (error.message.includes('Missing required fields') || error.message.includes('No idempotency key')) {
      logger.warn('Terminal validation error', { idempotencyKey, error: error.message });
      return res.status(400).send(`Bad Request: ${error.message}`);
    }
    
    // UPDATED: Refined error log with IDs
