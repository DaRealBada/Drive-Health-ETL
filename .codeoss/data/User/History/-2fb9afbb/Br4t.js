...
function shouldSample(idempotencyKey, auditRate) {
  ...
  // Create deterministic hash of the key
  const hash = crypto.createHash('md5').update(idempotencyKey).digest('hex');
  // Convert first 8 chars to integer and normalize to 0-1
...
module.exports = { validateEnvelope, computeIdempotencyKey, shouldSample, normalizePhone, processPayload };