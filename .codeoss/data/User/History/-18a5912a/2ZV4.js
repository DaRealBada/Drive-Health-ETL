...
app.post('/process-call', async (req, res) => {
  try {
    ...
    // NEW: Input validation. Check if call_id exists before processing.
    if (callData.call_id) {
      ...
      if (Math.random() <= 1.0) { 
        ...
        const datasetId = 'call_audits'; 
        const tableId = 'processed_calls';   

        // A variable to track if the insert was successful
        ...
        for (let i = 0; i < 3; i++) {
          try {
            ...
            await bigquery.dataset(datasetId).table(tableId).insert([rowToInsert]);
            
            // If it works, log success, mark as successful, and break the loop
            console.log(`Attempt ${i + 1}: Successfully inserted call ${callData.call_id}`);
            successful = true;
            break;
            ...
            // If this was the last attempt, re-throw the error to fail the whole process
            if (i === 2) {
              throw error;
            }
    ...
    // NEW: If the call_id is missing, throw a specific error.
    } else {
      throw new Error('Validation Failed: call_id is missing');
    }
